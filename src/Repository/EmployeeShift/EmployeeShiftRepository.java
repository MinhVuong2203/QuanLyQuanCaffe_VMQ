...
public class EmployeeShiftRepository implements IEmployeeShiftRepository {
    ...
    public List<EmployeeShift> getAllEmployeeShift() throws SQLException {
        ...
        try (Connection connection = jdbcUtils.connect();
            ...
            while (rs.next()) {
                ...
                LocalDateTime startTime = rs.getTimestamp("startTime") != null
                        ? rs.getTimestamp("startTime").toLocalDateTime()
                        : null;
                LocalDateTime endTime = rs.getTimestamp("endTime") != null
                ...
                EmployeeShift employeeShift = new EmployeeShift(shiftID, employeeID, startTime, endTime, hourWorked,
                        hourWage, salary, status);
    ...
    @Override
    public List<EmployeeShift> getShiftsByEmployeeIDBetweenDates(int employeeID, LocalDate fromDate, LocalDate toDate)
            throws SQLException {
        ...
        try (Connection connection = jdbcUtils.connect();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    double salary = rs.getDouble("salary");
                    String status = rs.getString("status");
                    EmployeeShift shift = new EmployeeShift(shiftID, employeeID, startTime, endTime, hourWorked,
                            hourWage, salary, status);
                    shiftList.add(shift);
                }
            }
    ...
    @Override
    public void addRegister(int employeeId, String startTime, String endTime, int hourWage, String status)
            throws SQLException {
    ...
    @Override
    public void resplayRegister(int employeeId, String startTimeOld, String endTimeOld, String startTimeNew,
            String endTimeNew) throws SQLException {
        if (employeeId <= 0 || startTimeOld == null || endTimeOld == null || startTimeNew == null
                || endTimeNew == null) {
            throw new IllegalArgumentException("Thông tin không hợp lệ");
        }
